...
public class ProjectInfo extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Cursor> {
    ...
    public void projectWebPage(View view) {
    ...
    public Loader<Cursor> onCreateLoader(int id, Bundle args) {
        ...
        return new CursorLoader(this,
                ProjectDb.KickEntry.buildProjectWithSerial(sl_number),
                null, null, null, null);
    }
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor data) {
        ...
        if (data != null && data.moveToFirst()) {
            ...
            int title_index = data.getColumnIndex(ProjectDb.KickEntry.KICK_TITLE);
            int backers_index = data.getColumnIndex(ProjectDb.KickEntry.KICK_BACKERS);
            int endtime_index = data.getColumnIndex(ProjectDb.KickEntry.KICK_END_TIME);
            int pleaged_index = data.getColumnIndex(ProjectDb.KickEntry.KICK_AMT_PLEDGED);
            int blurb_index = data.getColumnIndex(ProjectDb.KickEntry.KICK_BLURB);
            int by_index = data.getColumnIndex(ProjectDb.KickEntry.KICK_BY);
            int country_index = data.getColumnIndex(ProjectDb.KickEntry.KICK_COUNTRY);
            int currency_index = data.getColumnIndex(ProjectDb.KickEntry.KICK_CURRENCY);
            int location_index = data.getColumnIndex(ProjectDb.KickEntry.KICK_LOCATION);
            int state_index = data.getColumnIndex(ProjectDb.KickEntry.KICK_STATE);
            int type_index = data.getColumnIndex(ProjectDb.KickEntry.KICK_TYPE);
            int url_index = data.getColumnIndex(ProjectDb.KickEntry.KICK_URL);
            int percentage_funded_index = data.getColumnIndex(ProjectDb.KickEntry.KICK_PERCENTAGE_FUNDED);
